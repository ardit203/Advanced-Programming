...
## Auditory Exercise 6 (Maps)

#### 1. Chatbot (exam problem)

Implement a class `Chatbot` for managing the users and their interactions with the application. Each user can have multiple different sessions (conversations) with the chatbot (similar to the tabs on the left in the ChatGPT menu). For the class, define the following methods:

* `Chatbot(LLMModelDetails llmModelDetails, List<String> notSupportedFiles, int allowedAttachmentsSize)` – constructor with three arguments:

    * An object of the class `LLMModelDetails` which contains information about the prices for using the language model
    * `notSupportedFiles` – a list of strings that contain the extensions of the disallowed files for attaching to the chatbot (e.g. `[“.exe”,”.rar”]`)
    * `allowedAttachmentsSize` – the allowed size of the attached files in MB within a single interaction

* `public void addInteraction(String userId, String sessionId, String question, long timestampQuestion, String answer, long timestampAnswer, List<Attachment> attachments)` – the method adds a new interaction which the user `userId` has had within the session `sessionId`. The interaction is composed of the question `question` and the answer `answer`, and it also contains details about the time when the question was asked (`timestampQuestion`) and the time when the question was answered (`timestampAnswer`). Also, the method has the argument `attachments` which represents a list of attached files at the moment when the user asks the question.

    * With an exception of type `FileNotSupportedException`, prevent adding an interaction in which some of the attached files has an extension that is one of the forbidden extensions for the chatbot
    * With an exception of type `AttachmentsSizeExceededException`, prevent adding an interaction in which the total size of the attached files exceeds the maximum allowed size of attached files for the chatbot

* `void printConversation(String userId, String sessionId)` – a method that prints all interactions that the user `userId` has had within the session `sessionId`, sorted in ascending order according to the time of asking the question, and then according to the time of answering the question.

* `void printSessionDetails(String userId, String sessionId)` – a method that prints details about the session `sessionId` of the user `userId` (information about the number of interactions, total input tokens, total output tokens, total cost of all interactions in the session, total processing time of all interactions, total number of attachments in all interactions and total size of the attachments in the interactions).

* `void printUserDetails(String userId)` – a method that prints the details for all sessions of the user `userId` (using the method `printSessionDetails`). The sessions should be sorted in descending order according to the total number of interactions, and then according to the cost.

* `void longestProcessingTimeInteractions()` – a method that prints the interaction with the longest processing time for each user. The users should be sorted lexicographically according to the `ID` in ascending order. All messages in all sessions of the given user are taken into account.

* `void mostExpesiveInteractions()` – a method that prints the interaction with the highest cost for each user. The users should be sorted lexicographically according to the `ID` in ascending order. All messages in all sessions of the given user are taken into account.

Explanations:

* In the initial code, the classes `LLMModelDetails` and `Attachment` are given. These classes should not be changed. In the class `LLMModelDetails`, there is information about the model name, price per input and output token, and price per MB of attached file. In the class `Attachment`, there are the file name and file size in MB. Objects of these two classes are passed to the methods and constructors of the class `Chatbot` and they should be used for the logic of the problem.
* The number of tokens of one sentence is calculated the same as in the first problem.

    * Input tokens are the number of tokens used for the question
    * Output tokens are the number of tokens used for the answer
* The price of one interaction is calculated as:
  `input_tokens * input_token_price + output_tokens * output_token_price + attachments_size * price_per_MB_attachment`
* The processing time is the time passed from asking the question until generating the answer.
