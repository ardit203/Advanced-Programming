package Lab.Lab6.Task1;//package midterms.m1;
...
class University {
    ...
    public Map<String, Double> getAverageDifficultyPerDepartment() {
        return departments.stream()
                .collect(Collectors.toMap(
                        Department::getName,
                        d -> d.getCourses().stream().mapToInt(Course::getDifficulty).average().orElse(5.0)
                ));
    }
    ...
    public University mergeFourSmallestDepartments() {
        List<Department> lowest = departments.stream()
                .sorted(Comparator.comparing(d -> d.getCourses().stream().mapToInt(Course::getEnrolledStudents).sum()))
                .limit(4)
                .collect(Collectors.toList());

        departments.removeIf(lowest::contains);
        Department department = lowest.stream().reduce((left, right) -> {
                    String departmentName = String.join(" & ", List.of(left.getName(), right.getName()));
                    List<Course> courses = new ArrayList<>(left.getCourses());
                    courses.addAll(right.getCourses());
                    return new Department(departmentName, courses);
                })
                .orElseThrow();

        departments.add(department);

        return new University(departments);
    }
}
